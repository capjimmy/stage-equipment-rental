...
async function seedRental() {
    ...
    const orderRepo = dataSource.getRepository(entities_1.Order);
    const rentalRepo = dataSource.getRepository(entities_1.Rental);
    ...
    const order1 = await orderRepo.save({
        ...
        paymentStatus: entities_1.PaymentStatus.CONFIRMED,
        fulfillmentStatus: entities_1.FulfillmentStatus.CONFIRMED,
    });
    console.log(`✅ 주문 1 생성 완료 (ID: ${order1.id})`);
    const asset1 = product.assets[0];
    const rental1BlockedEnd = new Date(rental1End);
    rental1BlockedEnd.setDate(rental1BlockedEnd.getDate() + 1);
    ...
    const rental1 = await rentalRepo.save({
        ...
        status: entities_1.RentalStatus.CONFIRMED,
    });
    ...
    const order2 = await orderRepo.save({
        ...
        paymentMethod: 'bank_transfer',
        paymentStatus: entities_1.PaymentStatus.PENDING,
        fulfillmentStatus: entities_1.FulfillmentStatus.HOLD_PENDINGPAY,
    });
    ...
    if (product.assets.length > 1) {
        ...
        rental2BlockedEnd.setDate(rental2BlockedEnd.getDate() + 1);
        ...
        const rental2 = await rentalRepo.save({
            ...
            status: entities_1.RentalStatus.HOLD_PENDINGPAY,
        });
    ...
    const blockedPeriods = await rentalRepo
        ...
        statuses: [
            entities_1.RentalStatus.HOLD_PENDINGPAY,
            entities_1.RentalStatus.CONFIRMED,
            entities_1.RentalStatus.RENTED,
        ],
    })
...
//# sourceMappingURL=seed-rental.js.map